...
import utils
import graph
from load_data_seed_iv_1 import load_data
...
def evaluate(model, target_test_loader):
    ...
    with torch.no_grad():
        for eeg_target, eye_target, label_target in target_test_loader:
            ...
            conf_matrix += confusion_matrix(label_target_1D.cpu().numpy(), prediction.cpu().numpy(),
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, cls_weight, distill_weight, sub_id):
    ...
    model_filename = f'./model_parameters/seed_iv/model_parameters_session_{sub_id}.pth'
    os.makedirs(f'./model_parameters/seed_iv', exist_ok=True)
    ...
    for e in range(args.n_epoch):
        ...
        for mlen in range(n_batch):
            ...
            # if mlen % 10 == 0:
            #     model.visualization(eeg_target, eye_target, label_target, 'SEED-IV')
        ...
        np.savetxt(f'./train_log_seed_iv_session.csv', np_log, delimiter=',', fmt='%.6f')
...
def main(layer_num, distill_weight, cls_weight, seed):
    setup_seed(seed)
    all_test_results = []
    ...
    for i in range(1, 4):
        ...
        source_loader, target_train_loader, target_test_loader = load_data(test_id=i, BATCH_SIZE=batch_size)
        ...
        model.init_all()
        best_acc, best_confusion_matrix = train(source_loader, target_train_loader, target_test_loader, model, optimizer, cls_weight, distill_weight, i)
    ...
    cm_normalized = final_confusion_matrix.astype('float') / final_confusion_matrix.sum(axis=1)[:, np.newaxis]
    ...
    plt.tight_layout()
    plt.savefig('seed_iv_session_confusion_matrix.pdf', bbox_inches='tight')

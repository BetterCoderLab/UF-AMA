...
import utils
import graph
from load_data_seed_1 import load_data
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, cls_weight, distill_weight, sub_id):
    ...
    model_filename = f'./model_parameters/seed/model_parameters_session_{sub_id}.pth'
    ...
    for e in range(args.n_epoch):
        ...
        np.savetxt(f'./train_log_seed_session.csv', np_log, delimiter=',', fmt='%.6f')
...
def main(layer_num, distill_weight, cls_weight, seed):
    ...
    plt.tight_layout()
    plt.savefig('seed_session_confusion_matrix.pdf', bbox_inches='tight')

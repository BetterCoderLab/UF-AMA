...
import utils
import graph
from load_data_seed_iv import load_data
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, cls_weight, distill_weight, sub_id):
    ...
    model_filename = f'./model_parameters/seed_iv/model_parameters_subject_{sub_id}.pth'
    ...
    for e in range(args.n_epoch):
        ...
        np.savetxt(f'./train_log_seed_iv_subject.csv', np_log, delimiter=',', fmt='%.6f')
...
if __name__ == '__main__':
    ...
    for session in [1, 2, 3]:
        print("\nsession: " + str(session))
        final_confusion_matrix = main(session, 3, 1e-5, 1, 10)
    ...
    plt.tight_layout()
    plt.savefig('seed_iv_subject_confusion_matrix.pdf', bbox_inches='tight')
